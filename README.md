# Go API client for omnistrategosdk
...
```go
import omnistrategosdk "github.com/omnistrate/omnistrate-sdk-go"
```
...
For using other server than the one defined on index 0 set context value `omnistrategosdk.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), omnistrategosdk.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `omnistrategosdk.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), omnistrategosdk.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `omnistrategosdk.ContextOperationServerIndices` and `omnistrategosdk.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), omnistrategosdk.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), omnistrategosdk.ContextOperationServerVariables, map[string]map[string]string{
...
```go
auth := context.WithValue(context.Background(), omnistrategosdk.ContextAccessToken, "BEARER_TOKEN_STRING")
...
```go
auth := context.WithValue(context.Background(), omnistrategosdk.ContextBasicAuth, omnistrategosdk.BasicAuth{
	UserName: "username",
	Password: "password",
})
r, err := client.Service.Operation(auth, args)
