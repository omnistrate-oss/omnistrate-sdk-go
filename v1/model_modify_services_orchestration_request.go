/*
Omnistrate Registration API

REST API for Omnistrate Service Registration

API version: 2022-09-01-00
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package v1

import (
	"encoding/json"
	"fmt"
)

// checks if the ModifyServicesOrchestrationRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ModifyServicesOrchestrationRequest{}

// ModifyServicesOrchestrationRequest struct for ModifyServicesOrchestrationRequest
type ModifyServicesOrchestrationRequest struct {
	// ID of a Services Orchestration
	Id string `json:"id"`
	// base64 encoded content of services orchestration modify DSL
	OrchestrationModifyDSL string `json:"orchestrationModifyDSL"`
	// JWT token used to perform authorization
	Token string `json:"token"`
	AdditionalProperties map[string]interface{}
}

type _ModifyServicesOrchestrationRequest ModifyServicesOrchestrationRequest

// NewModifyServicesOrchestrationRequest instantiates a new ModifyServicesOrchestrationRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewModifyServicesOrchestrationRequest(id string, orchestrationModifyDSL string, token string) *ModifyServicesOrchestrationRequest {
	this := ModifyServicesOrchestrationRequest{}
	this.Id = id
	this.OrchestrationModifyDSL = orchestrationModifyDSL
	this.Token = token
	return &this
}

// NewModifyServicesOrchestrationRequestWithDefaults instantiates a new ModifyServicesOrchestrationRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewModifyServicesOrchestrationRequestWithDefaults() *ModifyServicesOrchestrationRequest {
	this := ModifyServicesOrchestrationRequest{}
	return &this
}

// GetId returns the Id field value
func (o *ModifyServicesOrchestrationRequest) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *ModifyServicesOrchestrationRequest) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *ModifyServicesOrchestrationRequest) SetId(v string) {
	o.Id = v
}

// GetOrchestrationModifyDSL returns the OrchestrationModifyDSL field value
func (o *ModifyServicesOrchestrationRequest) GetOrchestrationModifyDSL() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.OrchestrationModifyDSL
}

// GetOrchestrationModifyDSLOk returns a tuple with the OrchestrationModifyDSL field value
// and a boolean to check if the value has been set.
func (o *ModifyServicesOrchestrationRequest) GetOrchestrationModifyDSLOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.OrchestrationModifyDSL, true
}

// SetOrchestrationModifyDSL sets field value
func (o *ModifyServicesOrchestrationRequest) SetOrchestrationModifyDSL(v string) {
	o.OrchestrationModifyDSL = v
}

// GetToken returns the Token field value
func (o *ModifyServicesOrchestrationRequest) GetToken() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Token
}

// GetTokenOk returns a tuple with the Token field value
// and a boolean to check if the value has been set.
func (o *ModifyServicesOrchestrationRequest) GetTokenOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Token, true
}

// SetToken sets field value
func (o *ModifyServicesOrchestrationRequest) SetToken(v string) {
	o.Token = v
}

func (o ModifyServicesOrchestrationRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ModifyServicesOrchestrationRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["id"] = o.Id
	toSerialize["orchestrationModifyDSL"] = o.OrchestrationModifyDSL
	toSerialize["token"] = o.Token

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *ModifyServicesOrchestrationRequest) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"id",
		"orchestrationModifyDSL",
		"token",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varModifyServicesOrchestrationRequest := _ModifyServicesOrchestrationRequest{}

	err = json.Unmarshal(data, &varModifyServicesOrchestrationRequest)

	if err != nil {
		return err
	}

	*o = ModifyServicesOrchestrationRequest(varModifyServicesOrchestrationRequest)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "id")
		delete(additionalProperties, "orchestrationModifyDSL")
		delete(additionalProperties, "token")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableModifyServicesOrchestrationRequest struct {
	value *ModifyServicesOrchestrationRequest
	isSet bool
}

func (v NullableModifyServicesOrchestrationRequest) Get() *ModifyServicesOrchestrationRequest {
	return v.value
}

func (v *NullableModifyServicesOrchestrationRequest) Set(val *ModifyServicesOrchestrationRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableModifyServicesOrchestrationRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableModifyServicesOrchestrationRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableModifyServicesOrchestrationRequest(val *ModifyServicesOrchestrationRequest) *NullableModifyServicesOrchestrationRequest {
	return &NullableModifyServicesOrchestrationRequest{value: val, isSet: true}
}

func (v NullableModifyServicesOrchestrationRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableModifyServicesOrchestrationRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}

