/*
Omnistrate Fleet API

REST API for Omnistrate Fleet

API version: 2022-09-01-00
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package fleet

import (
	"encoding/json"
	"fmt"
)

// checks if the UpdateDeploymentCellWorkflowRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &UpdateDeploymentCellWorkflowRequest{}

// UpdateDeploymentCellWorkflowRequest struct for UpdateDeploymentCellWorkflowRequest
type UpdateDeploymentCellWorkflowRequest struct {
	// ID of the Host Cluster
	HostClusterID string `json:"hostClusterID"`
	// Update the status of the deployment cell workflow execution. You can pause, resume or retry a workflow.
	Status string `json:"status"`
	// JWT token used to perform authorization
	Token string `json:"token"`
	// ID of the Deployment Cell Workflow
	WorkflowID string `json:"workflowID"`
	AdditionalProperties map[string]interface{}
}

type _UpdateDeploymentCellWorkflowRequest UpdateDeploymentCellWorkflowRequest

// NewUpdateDeploymentCellWorkflowRequest instantiates a new UpdateDeploymentCellWorkflowRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewUpdateDeploymentCellWorkflowRequest(hostClusterID string, status string, token string, workflowID string) *UpdateDeploymentCellWorkflowRequest {
	this := UpdateDeploymentCellWorkflowRequest{}
	this.HostClusterID = hostClusterID
	this.Status = status
	this.Token = token
	this.WorkflowID = workflowID
	return &this
}

// NewUpdateDeploymentCellWorkflowRequestWithDefaults instantiates a new UpdateDeploymentCellWorkflowRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewUpdateDeploymentCellWorkflowRequestWithDefaults() *UpdateDeploymentCellWorkflowRequest {
	this := UpdateDeploymentCellWorkflowRequest{}
	return &this
}

// GetHostClusterID returns the HostClusterID field value
func (o *UpdateDeploymentCellWorkflowRequest) GetHostClusterID() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.HostClusterID
}

// GetHostClusterIDOk returns a tuple with the HostClusterID field value
// and a boolean to check if the value has been set.
func (o *UpdateDeploymentCellWorkflowRequest) GetHostClusterIDOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.HostClusterID, true
}

// SetHostClusterID sets field value
func (o *UpdateDeploymentCellWorkflowRequest) SetHostClusterID(v string) {
	o.HostClusterID = v
}

// GetStatus returns the Status field value
func (o *UpdateDeploymentCellWorkflowRequest) GetStatus() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Status
}

// GetStatusOk returns a tuple with the Status field value
// and a boolean to check if the value has been set.
func (o *UpdateDeploymentCellWorkflowRequest) GetStatusOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Status, true
}

// SetStatus sets field value
func (o *UpdateDeploymentCellWorkflowRequest) SetStatus(v string) {
	o.Status = v
}

// GetToken returns the Token field value
func (o *UpdateDeploymentCellWorkflowRequest) GetToken() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Token
}

// GetTokenOk returns a tuple with the Token field value
// and a boolean to check if the value has been set.
func (o *UpdateDeploymentCellWorkflowRequest) GetTokenOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Token, true
}

// SetToken sets field value
func (o *UpdateDeploymentCellWorkflowRequest) SetToken(v string) {
	o.Token = v
}

// GetWorkflowID returns the WorkflowID field value
func (o *UpdateDeploymentCellWorkflowRequest) GetWorkflowID() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.WorkflowID
}

// GetWorkflowIDOk returns a tuple with the WorkflowID field value
// and a boolean to check if the value has been set.
func (o *UpdateDeploymentCellWorkflowRequest) GetWorkflowIDOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.WorkflowID, true
}

// SetWorkflowID sets field value
func (o *UpdateDeploymentCellWorkflowRequest) SetWorkflowID(v string) {
	o.WorkflowID = v
}

func (o UpdateDeploymentCellWorkflowRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o UpdateDeploymentCellWorkflowRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["hostClusterID"] = o.HostClusterID
	toSerialize["status"] = o.Status
	toSerialize["token"] = o.Token
	toSerialize["workflowID"] = o.WorkflowID

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *UpdateDeploymentCellWorkflowRequest) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"hostClusterID",
		"status",
		"token",
		"workflowID",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varUpdateDeploymentCellWorkflowRequest := _UpdateDeploymentCellWorkflowRequest{}

	err = json.Unmarshal(data, &varUpdateDeploymentCellWorkflowRequest)

	if err != nil {
		return err
	}

	*o = UpdateDeploymentCellWorkflowRequest(varUpdateDeploymentCellWorkflowRequest)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "hostClusterID")
		delete(additionalProperties, "status")
		delete(additionalProperties, "token")
		delete(additionalProperties, "workflowID")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableUpdateDeploymentCellWorkflowRequest struct {
	value *UpdateDeploymentCellWorkflowRequest
	isSet bool
}

func (v NullableUpdateDeploymentCellWorkflowRequest) Get() *UpdateDeploymentCellWorkflowRequest {
	return v.value
}

func (v *NullableUpdateDeploymentCellWorkflowRequest) Set(val *UpdateDeploymentCellWorkflowRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableUpdateDeploymentCellWorkflowRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableUpdateDeploymentCellWorkflowRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableUpdateDeploymentCellWorkflowRequest(val *UpdateDeploymentCellWorkflowRequest) *NullableUpdateDeploymentCellWorkflowRequest {
	return &NullableUpdateDeploymentCellWorkflowRequest{value: val, isSet: true}
}

func (v NullableUpdateDeploymentCellWorkflowRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableUpdateDeploymentCellWorkflowRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


