/*
Omnistrate Fleet API

REST API for Omnistrate Fleet

API version: 2022-09-01-00
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package fleet

import (
	"encoding/json"
	"fmt"
)

// checks if the FleetModifyServicesOrchestrationRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &FleetModifyServicesOrchestrationRequest{}

// FleetModifyServicesOrchestrationRequest struct for FleetModifyServicesOrchestrationRequest
type FleetModifyServicesOrchestrationRequest struct {
	// ID of a Services Orchestration
	Id string `json:"id"`
	// base64 encoded content of services orchestration modify DSL
	OrchestrationModifyDSL string `json:"orchestrationModifyDSL"`
	// JWT token used to perform authorization
	Token string `json:"token"`
	AdditionalProperties map[string]interface{}
}

type _FleetModifyServicesOrchestrationRequest FleetModifyServicesOrchestrationRequest

// NewFleetModifyServicesOrchestrationRequest instantiates a new FleetModifyServicesOrchestrationRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewFleetModifyServicesOrchestrationRequest(id string, orchestrationModifyDSL string, token string) *FleetModifyServicesOrchestrationRequest {
	this := FleetModifyServicesOrchestrationRequest{}
	this.Id = id
	this.OrchestrationModifyDSL = orchestrationModifyDSL
	this.Token = token
	return &this
}

// NewFleetModifyServicesOrchestrationRequestWithDefaults instantiates a new FleetModifyServicesOrchestrationRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewFleetModifyServicesOrchestrationRequestWithDefaults() *FleetModifyServicesOrchestrationRequest {
	this := FleetModifyServicesOrchestrationRequest{}
	return &this
}

// GetId returns the Id field value
func (o *FleetModifyServicesOrchestrationRequest) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *FleetModifyServicesOrchestrationRequest) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *FleetModifyServicesOrchestrationRequest) SetId(v string) {
	o.Id = v
}

// GetOrchestrationModifyDSL returns the OrchestrationModifyDSL field value
func (o *FleetModifyServicesOrchestrationRequest) GetOrchestrationModifyDSL() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.OrchestrationModifyDSL
}

// GetOrchestrationModifyDSLOk returns a tuple with the OrchestrationModifyDSL field value
// and a boolean to check if the value has been set.
func (o *FleetModifyServicesOrchestrationRequest) GetOrchestrationModifyDSLOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.OrchestrationModifyDSL, true
}

// SetOrchestrationModifyDSL sets field value
func (o *FleetModifyServicesOrchestrationRequest) SetOrchestrationModifyDSL(v string) {
	o.OrchestrationModifyDSL = v
}

// GetToken returns the Token field value
func (o *FleetModifyServicesOrchestrationRequest) GetToken() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Token
}

// GetTokenOk returns a tuple with the Token field value
// and a boolean to check if the value has been set.
func (o *FleetModifyServicesOrchestrationRequest) GetTokenOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Token, true
}

// SetToken sets field value
func (o *FleetModifyServicesOrchestrationRequest) SetToken(v string) {
	o.Token = v
}

func (o FleetModifyServicesOrchestrationRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o FleetModifyServicesOrchestrationRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["id"] = o.Id
	toSerialize["orchestrationModifyDSL"] = o.OrchestrationModifyDSL
	toSerialize["token"] = o.Token

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *FleetModifyServicesOrchestrationRequest) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"id",
		"orchestrationModifyDSL",
		"token",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varFleetModifyServicesOrchestrationRequest := _FleetModifyServicesOrchestrationRequest{}

	err = json.Unmarshal(data, &varFleetModifyServicesOrchestrationRequest)

	if err != nil {
		return err
	}

	*o = FleetModifyServicesOrchestrationRequest(varFleetModifyServicesOrchestrationRequest)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "id")
		delete(additionalProperties, "orchestrationModifyDSL")
		delete(additionalProperties, "token")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableFleetModifyServicesOrchestrationRequest struct {
	value *FleetModifyServicesOrchestrationRequest
	isSet bool
}

func (v NullableFleetModifyServicesOrchestrationRequest) Get() *FleetModifyServicesOrchestrationRequest {
	return v.value
}

func (v *NullableFleetModifyServicesOrchestrationRequest) Set(val *FleetModifyServicesOrchestrationRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableFleetModifyServicesOrchestrationRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableFleetModifyServicesOrchestrationRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableFleetModifyServicesOrchestrationRequest(val *FleetModifyServicesOrchestrationRequest) *NullableFleetModifyServicesOrchestrationRequest {
	return &NullableFleetModifyServicesOrchestrationRequest{value: val, isSet: true}
}

func (v NullableFleetModifyServicesOrchestrationRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableFleetModifyServicesOrchestrationRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


