/*
Omnistrate Fleet API

REST API for Omnistrate Fleet

API version: 2022-09-01-00
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package fleet

import (
	"encoding/json"
	"fmt"
)

// checks if the FleetResourceInstanceCustomWorkflowRequest2 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &FleetResourceInstanceCustomWorkflowRequest2{}

// FleetResourceInstanceCustomWorkflowRequest2 struct for FleetResourceInstanceCustomWorkflowRequest2
type FleetResourceInstanceCustomWorkflowRequest2 struct {
	// Input parameters for the custom workflow. System workflow operations do not require user supplied parameters.
	RequestParams interface{} `json:"requestParams,omitempty"`
	// The resource ID.
	ResourceId string `json:"resourceId"`
	AdditionalProperties map[string]interface{}
}

type _FleetResourceInstanceCustomWorkflowRequest2 FleetResourceInstanceCustomWorkflowRequest2

// NewFleetResourceInstanceCustomWorkflowRequest2 instantiates a new FleetResourceInstanceCustomWorkflowRequest2 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewFleetResourceInstanceCustomWorkflowRequest2(resourceId string) *FleetResourceInstanceCustomWorkflowRequest2 {
	this := FleetResourceInstanceCustomWorkflowRequest2{}
	this.ResourceId = resourceId
	return &this
}

// NewFleetResourceInstanceCustomWorkflowRequest2WithDefaults instantiates a new FleetResourceInstanceCustomWorkflowRequest2 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewFleetResourceInstanceCustomWorkflowRequest2WithDefaults() *FleetResourceInstanceCustomWorkflowRequest2 {
	this := FleetResourceInstanceCustomWorkflowRequest2{}
	return &this
}

// GetRequestParams returns the RequestParams field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *FleetResourceInstanceCustomWorkflowRequest2) GetRequestParams() interface{} {
	if o == nil {
		var ret interface{}
		return ret
	}
	return o.RequestParams
}

// GetRequestParamsOk returns a tuple with the RequestParams field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *FleetResourceInstanceCustomWorkflowRequest2) GetRequestParamsOk() (*interface{}, bool) {
	if o == nil || IsNil(o.RequestParams) {
		return nil, false
	}
	return &o.RequestParams, true
}

// HasRequestParams returns a boolean if a field has been set.
func (o *FleetResourceInstanceCustomWorkflowRequest2) HasRequestParams() bool {
	if o != nil && !IsNil(o.RequestParams) {
		return true
	}

	return false
}

// SetRequestParams gets a reference to the given interface{} and assigns it to the RequestParams field.
func (o *FleetResourceInstanceCustomWorkflowRequest2) SetRequestParams(v interface{}) {
	o.RequestParams = v
}

// GetResourceId returns the ResourceId field value
func (o *FleetResourceInstanceCustomWorkflowRequest2) GetResourceId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ResourceId
}

// GetResourceIdOk returns a tuple with the ResourceId field value
// and a boolean to check if the value has been set.
func (o *FleetResourceInstanceCustomWorkflowRequest2) GetResourceIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ResourceId, true
}

// SetResourceId sets field value
func (o *FleetResourceInstanceCustomWorkflowRequest2) SetResourceId(v string) {
	o.ResourceId = v
}

func (o FleetResourceInstanceCustomWorkflowRequest2) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o FleetResourceInstanceCustomWorkflowRequest2) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if o.RequestParams != nil {
		toSerialize["requestParams"] = o.RequestParams
	}
	toSerialize["resourceId"] = o.ResourceId

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *FleetResourceInstanceCustomWorkflowRequest2) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"resourceId",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varFleetResourceInstanceCustomWorkflowRequest2 := _FleetResourceInstanceCustomWorkflowRequest2{}

	err = json.Unmarshal(data, &varFleetResourceInstanceCustomWorkflowRequest2)

	if err != nil {
		return err
	}

	*o = FleetResourceInstanceCustomWorkflowRequest2(varFleetResourceInstanceCustomWorkflowRequest2)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "requestParams")
		delete(additionalProperties, "resourceId")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableFleetResourceInstanceCustomWorkflowRequest2 struct {
	value *FleetResourceInstanceCustomWorkflowRequest2
	isSet bool
}

func (v NullableFleetResourceInstanceCustomWorkflowRequest2) Get() *FleetResourceInstanceCustomWorkflowRequest2 {
	return v.value
}

func (v *NullableFleetResourceInstanceCustomWorkflowRequest2) Set(val *FleetResourceInstanceCustomWorkflowRequest2) {
	v.value = val
	v.isSet = true
}

func (v NullableFleetResourceInstanceCustomWorkflowRequest2) IsSet() bool {
	return v.isSet
}

func (v *NullableFleetResourceInstanceCustomWorkflowRequest2) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableFleetResourceInstanceCustomWorkflowRequest2(val *FleetResourceInstanceCustomWorkflowRequest2) *NullableFleetResourceInstanceCustomWorkflowRequest2 {
	return &NullableFleetResourceInstanceCustomWorkflowRequest2{value: val, isSet: true}
}

func (v NullableFleetResourceInstanceCustomWorkflowRequest2) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableFleetResourceInstanceCustomWorkflowRequest2) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


